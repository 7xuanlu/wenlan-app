...
| remaining direct route gaps | `/api/config/skip-apps`, `/api/context`, `/api/debug/pipeline`, `/api/memory/entities/{entity_id}/observations`, `/api/memory/link-entity`, `/api/memory/relations`, `/api/ping`, `/api/steep`, `/ws/updates` | classified in `docs/superpowers/refactor/2026-06-29-wenlan-app-remaining-route-gaps.md` | avoid raw wrappers for superseded, agent/operator, and graph-authoring routes; design first if product needs them | not required for desktop P0/P1 parity without a designed surface |
